...
### ⚠️ 结论修正（2026-09-10，3000 行压测）

上面的「值得做但不紧急」是**按 372 行外推**得出的——斜率 0.048 秒/行在
3000 行上就是 **2 分 24 秒纯写入**。实测确认，见 `docs/STRESS_TEST.md`。

给映射器加计时桩后，3000 行分段实测：
...
即 **48 ms/行、约 4 ms/格**，与上面「180 次/秒」的对照数字吻合。

**本项应重新定级为「值得做且已到期」**：2 分半是用户点完「开始填充」后
干等的时间，而本程序的进度条是**不定式**的（README 明说不代表百分比），
用户无法区分「在跑」和「卡死」。

**另有一个已验证的免费优化**：在 `$excel.DisplayAlerts = $false` 之后加
`ScreenUpdating = $false` 与 `EnableEvents = $false`、保存前还原，
**写循环快 27%、总耗时快 25%**，且产物**逐格完全一致**
（141,564 格比对 `valueDiffs=0` `alignDiffs=0`，OOXML 部件与条件格式覆盖全等）。
交错 A/B 与证伪过程见 `docs/STRESS_TEST.md` 第 3 节。
该变体**刻意未动 `Calculation`**——那会影响公式重算，需单独验证。

> 追加写入功能（`ad5204a9`）复核：写入路径**结构未变**，仍是 `Set-CellValue`
> 逐格写、仍无这两个开关，故以上结论对它同样成立。
...
### 现场证据（2026-09-10，3000 行压测）

本项预言的「残留 `EXCEL.EXE` 进程」**已实际观测到 3 例**：
运行结束后有 3 个 `EXCEL.EXE`（70–115 MB）卡住 **20–40 分钟不退出**，
`StartTime` 精确落在 1500 行与 3000 行的映射运行窗口内。
...
| 1500 行交错 A/B | 4 | 0 |
| 3000 行 | 1 | 0 |
| 读回工具 `read_out.ps1` 连跑 | 5 | 0 |

即约 **17 次调用中 3 次**，**无法按需复现**；3 例都出现在大规模运行之后，
500 行无一例，提示与数据量相关。

**归因未定，不要据此断定是产品缺陷**：这些运行与我的读回工具
（`read_out.ps1`、`diff_two.ps1`，同样会新开 Excel 实例）时间窗重叠。
单独连跑 `read_out.ps1` 五次，其新开的实例都自行退干净、PID 集合精确回到基线。
但反过来说，**也无法排除产品侧**。

建议排查方向：`finally` 清理路径（`$sourceUsed`/`$sourceWs`/`$targetWs`/
`$sourceWb`/`$targetWb` 之外的中间 RCW，含 `Find-TargetColumns`、
`Find-TargetSheet` 里的单元格对象），并在第 5 项的批量方案落地后复测——
批量写入会顺带消除绝大部分逐格 RCW。详见 `docs/STRESS_TEST.md` 第 5 节。
...
| 中→**高** | 5. 逐格写入 | 3000 行实测**纯写入 144 s**，总耗时 2.5 分钟；先落已验证的免费优化（关 `ScreenUpdating`/`EnableEvents`，−25%，产物逐格一致），再做批量写入 |
| 中 | 4. 源表表头行数 | 需先确认业务实际 |
| 低 | 6. COM 泄漏 | 已观测到 3 例残留 `EXCEL.EXE`（归因未定，无法按需复现）；第 5 项做完后大部分自动消失 |
| — | 规模与进程压测 | 3000 行正确性通过，详见 `docs/STRESS_TEST.md` |
| 低 | 7. 只读打开 | 需实测验证 |
