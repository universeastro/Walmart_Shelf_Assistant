...
4. **输出扩展名**：Req03 冻结的「必须 `.xls`」（R03-06）不适用于 Req04——B 是 `.xlsx`，
   Req04 输出应跟随 B 的格式。请写入接口约定，避免沿用 03 规则误拒。
5. ~~第二个视频仍未出现~~ —— **本项作废**：`QQ20260914-130136.mp4` 已于 2026-09-14 13:04
   出现在 `文件/04/`（61,539,852 字节，md5 `ca88dda8357f7d2891bf6ac27932807c`），
   见文末「素材状态更正」。
6. **追加行是否要写 SKU（B 表 D 列）**：三版人工结果都没写 D 列，但 B 模板既有第 7–9 行的
   D 列**是有值的**。导出块若不带 SKU，追加行无法对应回商品、也无法事后复核。
   请确认七条映射之外是否还需写 D。**这是待确认项，不是缺陷判定。**
