...
| T37 | Claude | 待开始 | 独立需求、边界、回归、文档审查；结论写入 `HANDOFF.md` | T36；专项验证完成 |
| T38 | Codex | 待开始 | 整合、修复、完整回归、`AGENTS.md`/文档、提交 | T37 通过；用户授权提交/后续合并 |
...
- T34 仍不得开工，直到 T33 解除：输出扩展名、Append/Replace、空行准入、数值口径，以及“尺寸是否使用 A 表英寸换算列”完成确认；另需明确目标新批次是否写 SKU、CR 必须写源 Q 数值。SKU 连接、仅可见行、重量双写、A/B 路径和两个视频状态已解决。
