...
- Req04 已确认仅处理可见行（隐藏/筛选行跳过）；默认只允许 `Append`，确需覆盖时才允许显式 `Replace` 并二次确认。
...
- T33 已解除：输出扩展名跟随 B；尺寸使用 A 表 K/L/M 英寸列；价格 I/Q 写入 J/CR；目标新批次不写 B 的 D 列。SKU 连接、仅可见行、重量双写、Append/Replace、空行准入和数值失败口径均已解决；T34 现进入实现阶段。
