...
> 当前阶段：**04 并行功能**（分支 `feature/req04-parallel`）。
> T01–T30 为 01/02/03 阶段记录，保留作回归依据，编号不复用。
...
| T33 | Codex | 已完成 | `文件/04/` 素材登记、测试夹具、`PROJECT.md`、`TASKS.md`、`HANDOFF.md` | T32；A/B、行关联、输出、写入口径已冻结 |
| T34 | Codex | 已完成 | `excel_mapper.ps1`、Req04 映射配置 | T32/T33；字段契约已冻结并通过专项验证 |
| T35 | Codex | 已完成 | `app.py`、`README.md`、构建/依赖配置 | T34 接口稳定，Req04 页面与扩展名约束已验证 |
| T36 | Codex | 已完成 | `tests/`（Req04 专项与 01/02/03 回归） | T34/T35；真实 A/B、边界与回归全部通过 |
| T37 | Claude | 已完成 | 独立需求、边界、回归、文档审查；结论写入 `HANDOFF.md` | T36；审查阻断项已修复并复验 |
| T38 | Codex | 已完成 | 整合、修复、完整回归、文档与提交 | T37 通过；分支可交付 |
...
- Req04 的导出范围由用户选择，默认 `Visible`，也支持 `All`；Visible 模式跳过隐藏/筛选行。默认只允许 `Append`，确需覆盖时才允许显式 `Replace` 并二次确认。
- Req04 行准入：两张源表均须有非空 SKU，按 SKU 连接；缺失、重复或无法跨表匹配时直接报错。全部业务字段为空的行跳过；任一业务字段非空即可写入，单字段为空只保持对应目标字段为空。
- Req04 数值转换失败采用正式方案：可解析数字写数值，失败按原文本写入并在 JSON/GUI 中记录警告（SKU、源字段、原始值、目标字段）；不因单个字段转换失败而中止整批。
- Req04 格式契约：J/K/AN/AP/AR/AT/CR 新写入单元格统一使用水平对齐“常规”（`1`）与 `0.00_);[红色](0.00)` 数值格式；源业务字段出现 Excel 错误值（如 `#N/A`）时拒绝整批且不生成输出，普通文本转换失败仍按文本写入并报警告。
- 视频/模板已确认 `AP=Height`、`AR=Weight`、`AT=Width`；更新后的标准文档已确认 `宽 -> AT`。
- T33 已解除：输出扩展名跟随 B；尺寸使用 A 表 K/L/M 英寸列；价格 I/Q 写入 J/CR；目标新批次不写 B 的 D 列。SKU 连接、可选导出范围、重量双写、Append/Replace、空行准入和数值失败口径均已解决；T34 已完成。

> 当前阶段：**03 并行功能**（分支 `feature/req03-parallel`，基线 `14ac950`）。
> T01–T22 为已关闭的 01/02 阶段记录，保留作回归依据，编号不复用。
> 03 阶段需求与验收标准见 `PROJECT.md`；R03-01、R03-02、R03-06 已由用户裁决，
> R03-03–R03-05 已由视频直接验证（见 `HANDOFF.md`）。
...
| T24 | Claude | 已完成 | 只读需求、素材、视频、验收边界与接口冲突审查；结论写入 `HANDOFF.md` | T23；R03-03–R03-05 已有视频证据 |
| T25 | Codex | 已完成 | `excel_mapper.ps1`（Req03 profile、六字段映射、目标表/末行识别） | T24；真实 A/B 首批与追加通过，主线/Req02 回归通过 |
| T26 | Codex | 已完成 | `app.py`、`README.md`（第三方案页、路径与导出范围记忆、默认输出扩展名） | T25 接口稳定；既有 43 项 GUI/配置测试通过 |
...
- **写入语义：默认 `Append`，显式 `-WriteMode Replace` 才覆盖。** 空目标从第 2 行写入；
  ...
  该口径对 03 同样适用（R03-04：沿用 01/02 的「隔三行」定义）。
...
- GUI 侧 `PROFILE_LABELS` 增 `"支线 03": "Req03"`；方案页、A/B/输出路径、导出范围按方案独立记忆；
  03 默认导出范围「仅可见行」（R03-02）。
- 03 的输出必须沿用 B 的 `.xls` 格式（R03-06，用户已于 2026-09-12 确认）；GUI 与映射器
  均拒绝其他输出扩展名。
