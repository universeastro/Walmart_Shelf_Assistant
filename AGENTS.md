...
- 主线 01 与支线 02 仍主要依赖合成样本；支线 03 已用真实 `文件/03/A模板02.xls` 和
  `XPY沃尔玛价格计算.xls` 验证 306 个可见数据行、1836 个写入单元格、数值类型、公式保护、
  非目标工作表不变及原模板哈希不变。仍未经过用户对最终业务产物的人工验收。
- `tests/verify_req02.ps1` 的 JSON 输出已显式设为 UTF-8，已覆盖 Windows CP936 默认控制台；
...
**主线及既有方案的新写入 Color 单元格设为左对齐（-4131），其他映射字段仍为填充（5）。**
Color 按目标表头名称识别，不按固定列号。支线 04 的七个 Req04 映射字段例外，统一设为 Excel“水平对齐：常规”（1）和模板数值格式 `0.00_);[红色](0.00)`。
表头、旧数据、未写入单元格不改。对齐测试须按受影响方案实跑验证。
...
- `tests/verify_alignment.ps1` —— 断言主线写入单元格为水平对齐「填充」，
